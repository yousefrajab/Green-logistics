<x-guest-layout>
    <!-- الترحيب ومقدمة التسجيل للجمعية أو المتبرع أو السائق -->
    <div class="space-y-2">
        <h2 class="text-3xl font-extrabold tracking-tight text-slate-900">إنشاء حساب جديد</h2>
        <p class="text-sm leading-7 text-slate-500">انضم إلينا اليوم وساهم في إنقاذ وحفظ الطعام الفائض.</p>
    </div>

    <!-- واجهة التسجيل المجهزة بـ Alpine.js للتحكم الديناميكي بالحقول -->
    <div x-data="{ role: 'donor' }" class="w-full">
        <form method="POST" action="{{ route('register') }}" class="space-y-5 pt-4">
            @csrf

            <!-- ================= NAME ================= -->
            <div>
                <x-input-label for="name" :value="__('الاسم الشخصي أو اسم المسؤول')" class="mb-2 font-semibold" />
                <x-text-input id="name" class="input-modern block w-full rounded-xl border-slate-200 py-3.5 focus:border-emerald-500 focus:ring-emerald-500" type="text" name="name" :value="old('name')" required autofocus placeholder="اكتب اسم المسؤول" />
            ...
            <!-- ================= ACCOUNT TYPE (مطابق لـ Screenshot 12) ================= -->
            <div>
            ...
            <!-- إحداثيات الموقع (مخفية مؤقتاً للتطوير) -->
